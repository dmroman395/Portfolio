...
function ProjectCard({title, img, link, code, tech, id, side}) {
    ...
    return (
        <div className={`relative md:sticky transition-all ease-in-out px-8 py-16 divide-zinc-50 h-screen flex flex-grow top-0 md:w-[50%]`} id={id}>
            <div ref={project} className={`${side === 'left' ? 'hideProjectCardLeft' : 'hideProjectCardRight'} flex flex-col gap-y-5 w-full h-fit self-center`}>
                ...
                <ul className="flex flex-wrap justify-center my-3 text-slate-100">
                    {techList}
                </ul>
                <div className="flex justify-around">
                    {link ? <a href={link} target='_blank' rel="noreferrer" className='underline greenText hover:cursor-pointer'>View Live</a> : null}
